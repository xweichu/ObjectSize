...
def read_partial_data_from_objects_t(prefix, length, obj_size, tm):
    ...
    while True:
        ...
        try:
            ...
            print(prefix + ',' + str(i) + ',' + str((i)*obj_size / (time.time() -start_time )/ 1024/1024))
            return str(e)

    print(prefix + ',' + str(i) + ',' + str((i)*obj_size / (time.time() -start_time )/ 1024/1024))


def read_partial_data_from_objects(length, obj_size, thread_num, tm):

    process_target = read_partial_data_from_objects_t
    processes = []

    for i in range(thread_num):
        p = Process(target=process_target, args=('Thread_' + str(i),length, obj_size, tm))
        processes.append(p)
        p.start()
...
time.sleep(10)
populate_objects(int(sys.argv[1]), 64, 120)
time.sleep(10)
read_partial_data_from_objects(int(sys.argv[2]),int(sys.argv[1]) , 64, 120)
