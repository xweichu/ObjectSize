...
f = open('./test', 'rb')
bts = f.read()
re = write_data(bts,'test','scbench')
print(re)
...
for i in range(worker_num):
    data_offset = 1048576 * 4 * i
    p = Process(target=process_target, args=(obj_name, data_len, data_offset, ceph_pool))
    processes.append(p)
    p.start()
